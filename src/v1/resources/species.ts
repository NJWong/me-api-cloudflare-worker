...
const router = Router({ base: '/api/v1/species' });
...
router.get('/', async (request: IRequest, env: Env) => {
	...
	const countResult = await connection.execute('SELECT COUNT(*) FROM species');
...
router.get('/:id', async (request: IRequest, env: Env) => {
	...
	const query = `SELECT species.id, species.name FROM species WHERE species.id = ${id}`;
